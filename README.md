...
This project aims to analyse and predict credit card default risk to help financial institutions make informed decisions, minimise losses, and improve customer strategies.
...
- **What customer groups are most at risk?** Identifying specific demographic segments or behavioural patterns that correlate with higher default rates.
...
- **Hypothesis 1: Payment behaviour (e.g., consistent delays) significantly influences credit default.**
    - **How we checked it:** Explored through descriptive statistics, correlation analysis, and feature importance derived from machine learning models. Visualised using heatmaps and highlight tables.
- **Hypothesis 2: Demographic factors (Gender, Education, Marital Status, Age) are associated with credit default.**
    - **How we checked it:** Tested using statistical methods like Chi-Square tests (for categorical variables like Gender, Education, Marriage) and Mann-Whitney U tests (a non-parametric alternative to t-tests for continuous variables like Age) to determine if there are statistically significant differences between defaulters and non-defaulters. Visualised using stacked bar charts and box plots.
...
4.  **Feature Engineering:** Creating new, more insightful features from existing data (e.g., `Total Bill Amount`, `Payment to Bill Ratio`, `Payment Delay Count`, `High Utilisation`).
5.  **Machine Learning (ML):** Training and evaluating baseline predictive models.
6.  **Dashboarding:** Creating interactive visualisations in Tableau to present insights and model performance.
7.  **Conclusions & Recommendations:** Summarising key findings and providing actionable business recommendations.
...
  - **Visuals:** A large **KPI** shows the overall default rate of 22.12%, immediately answering "What is the scale of the problem?". Simple **bar charts** show overall client counts by gender and age, giving a quick demographic overview.
...
  - **Visuals:** A **Heatmap** visualises average payment delays across months for defaulters vs. non-defaulters, quickly highlighting problematic periods. A **Scatter Plot** compares total bill vs. total payment, revealing patterns in financial activity. A **Box Plot** shows the distribution of credit limits for defaulters vs. non-defaulters. A **Highlight Table** combines payment delay count and high utilisation to pinpoint the riskiest client segments, directly answering "Which features most influence credit default?" from a behavioural standpoint.
...
- **Tableau:** Used for creating interactive and insightful dashboards.
    - **Interactive Visualisations:** Bar charts, stacked bar charts, histograms, scatter plots, box plots, heatmaps, and highlight tables.
    - **Filters and Parameters:** Enabled dynamic exploration of the data by users.
    - **Colour Coding:** Consistent and colourblind-friendly palettes were used to enhance readability and highlight key distinctions (e.g., Defaulters vs. Non-Defaulters).

## Ethical Considerations

In developing this credit risk prediction solution, ethical considerations were paramount:

- **Fairness and Bias:** We acknowledge that historical data can contain biases. While direct demographic features like Gender and Age were included for analysis, careful consideration was given to their interpretation to avoid perpetuating discriminatory practices. The models are intended to identify risk based on financial behaviour, not to unfairly penalise specific groups.
- **Data Privacy:** The dataset used is anonymised. In a real-world scenario, strict adherence to data privacy regulations (e.g., GDPR, CCPA) would be essential, ensuring client data is protected and used only for its intended purpose.
...
- **Consistent Colour Coding:** A consistent, colourblind-friendly palette (e.g., blue for "No Default," orange for "Default") is used across all relevant charts, making it easy to distinguish between groups at a glance.
...
- **Diverse Chart Types:** A variety of chart types were chosen to best represent different data relationships:
  ...
    - **Histograms:** For visualising the distribution of continuous data like Age.
  ...
    - **Highlight Tables:** To show average metrics across combinations of categories, with colour highlighting risk levels.
...
- **Optimise** models through hyperparameter tuning (e.g., GridSearchCV, RandomizedSearchCV) for baseline models to achieve optimal performance.
